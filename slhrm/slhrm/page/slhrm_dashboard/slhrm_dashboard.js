	...
	frappe.call({
		...
		callback: function (r) {
			if (r.message) {
				...
				todayHtml +=
					'<tr><td>Present</td><td class="text-right"><span class="indicator-pill green">' +
					data.today.present +
					"</span></td></tr>";
				todayHtml +=
					'<tr><td>Absent</td><td class="text-right"><span class="indicator-pill red">' +
					data.today.absent +
					"</span></td></tr>";
				todayHtml +=
					'<tr><td>Half Day</td><td class="text-right"><span class="indicator-pill orange">' +
					data.today.half_day +
					"</span></td></tr>";
				todayHtml +=
					'<tr><td>On Leave</td><td class="text-right"><span class="indicator-pill blue">' +
					data.today.on_leave +
					"</span></td></tr>";
				todayHtml +=
					'<tr><td>Holiday</td><td class="text-right"><span class="indicator-pill gray">' +
					data.today.holiday +
					"</span></td></tr>";
				todayHtml += "</table>";
				...
				if (data.recent && data.recent.length > 0) {
					let recentHtml =
						'<table class="table table-striped"><thead><tr><th>Employee</th><th>Date</th><th>Status</th><th>Check In</th><th>Check Out</th><th>Hours</th></tr></thead><tbody>';
					data.recent.forEach(function (row) {
						...
						recentHtml +=
							...
							"</td><td>" +
							row.date +
							'</td><td><span class="indicator-pill ' +
							statusClass +
							">" +
							row.status +
							"</span></td><td>" +
					...
					$(
						...
						'<div class="text-muted">No attendance records found. <a href="/app/attendance-dashboard/new">Create your first record</a></div>'
					);
				}
			}
		},
